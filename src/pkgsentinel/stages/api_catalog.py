...
PYTHON_APIS: dict[str, AttackDimension] = {
    ...
    #
    # 2026-05-06 메모: expanduser/Path.home 일시 제외 시도 → 회복.
    #   합성 fixture eval 에서 ssh-key-theft / wallet-exfil / credential-paths-multifile
    #   등 8 FN 발생 (R 0.98 → 0.87). 이 fixture 들이 expanduser('~/.ssh/...') 패턴
    #   사용 → 제외 시 SP-001 (Credential exfiltration) 매칭 실패.
    #   pandas/io/common.py 의 read_csv("~/file") FP 는 별도 처리 필요
    #   (예: SP-001 의 broad-purpose 카테고리 가드 + INFO source 강도 판정).
